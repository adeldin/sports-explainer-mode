    ...
    return {
      ...
      beginner: {
        simple: success
          ...
          : `Incomplete pass.`,
        whyItMatters: success && yards >= (play.metadata?.yardsToGo || 10)
          ? `This gained a first down!`
          : `${(play.metadata?.yardsToGo || 10) - yards} yards needed for first down.`,
      ...
      intermediate: {
        simple: `${yards}-yard ${success ? 'completion' : 'incompletion'}. ${play.metadata?.down}${play.metadata?.down ? ' & ' : ''}${play.metadata?.yardsToGo || ''}`,
  ...
  private explainRun(play: FootballPlay) {
    ...
    return {
      ...
      beginner: {
        ...
        whyItMatters: yards >= (play.metadata?.yardsToGo || 10) ? `First down!` : `${(play.metadata?.yardsToGo || 10) - yards} yards to go.`,
