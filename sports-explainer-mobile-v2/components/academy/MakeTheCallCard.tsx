// MakeTheCallCard — the universal judgment-quiz card. Copies the FormationQuizCard mechanic (level
// pills, 4 animated options, reveal + explanation + Next, no-repeat cycling) but is TEXT-ONLY (no
// diagram) and pools BY SPORT from lib/makeTheCall. Deltas vs FormationQuizCard (per Gate 2):
//   (1) props { sportKeys, streak, onCorrect, onWrong } (matches the live QuizCard — pools by sport);
//   (2) pool = buildScenarioPool(sportKeys, level), regenerated on sportKeys OR level change;
//   (3) NO FormationDiagram — render question.q as plain text above the options;
//   (4) reveal adds an optional muted "Watch for:" line when the scenario has watchFor;
//   (5) empty state when no authored content exists for the current level/sport combo.
// QuizOption is COPIED from the live QuizCard so the shipped Academy quiz is left completely untouched.
...
// Fisher–Yates shuffle of a scenario's options, remapping the answer index (once per question).
function shuffleOptions(q: MakeTheCallScenario): { options: string[]; answer: number } {
...
export default function MakeTheCallCard({ sportKeys, streak, onCorrect, onWrong }: Props) {
  const { theme } = useTheme();
  const styles = useMemo(() => makeStyles(theme), [theme]);
  const { level, setLevel } = useAppState();

  // The pool is the authored scenarios for the current sport(s) at the current level; regenerated on
  // sportKeys OR level change. De-duped by logical bank inside buildScenarioPool (soccer's four keys
  // collapse to one). 0-length pools (e.g. kid/expert before content lands) render the empty state.
  ...
  // Fresh pool + question whenever the sport set OR level changes. sportKeys.join(',') keeps the dep
  // value-stable (a new array identity each render would otherwise re-run this every render).
  useEffect(() => {
    const next = buildScenarioPool(sportKeys, level);
    setPool(next);
    recent.current = [];
  ...
  return (
    <Animated.View style={[styles.card, cardStyle]}>
      {/* Level picker — tapping re-pools at the new level (kid/expert may be empty until content lands). */}
      ...
      {question ? (
        <>
          {/* TEXT-ONLY question: the situation + question, no diagram (v1). */}
          ...
          {answered && (
            <Animated.View style={revealStyle}>
              ...
              {question.watchFor ? <Text style={styles.watchFor}>Watch for: {question.watchFor}</Text> : null}
              {pool.length > 1 && (
        ...
        <Text style={styles.emptyText}>No scenarios here yet — try another level.</Text>
      )}
    </Animated.View>
  );
}
...
const makeStyles = (t: Theme) => StyleSheet.create({
  ...
  emptyText: { color: t.textSecondary, fontSize: 15, lineHeight: 22, textAlign: 'center', paddingVertical: 28 },
  ...
  // The one new bit of UI vs the Formation card: a muted secondary caption for watchFor (when present).
  watchFor: { color: t.textMuted, fontSize: 13, fontStyle: 'italic', lineHeight: 19, marginTop: 10 },
