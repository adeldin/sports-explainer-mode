...
import { SIGNAL_DECODER, poolForLevel, signalSportForKeys, SignalScenario } from '../../lib/signalDecoder';
...
export default function SignalDecoderGame({ sportKeys, categoryEmoji }: AcademyGameProps) {
  ...
  const signalSport = signalSportForKeys(sportKeys);
  const bank = signalSport ? SIGNAL_DECODER[signalSport] : [];
  const pool = useMemo(
  ...
  const comboLabel =
    ...
    : 'Read the official, make the call! 🚩';
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        {!scenario ? (
          // Dead path (supportedSports keeps signal-less categories away) — guarded
          // anyway, because a blank game reads as a broken app.
          <View style={styles.card}>
            <Text style={styles.emptyText}>No signals for this sport yet — try another category.</Text>
          </View>
        ) : (
          <>
            ...
            {/* The teaching beat: correct/incorrect tag + tier chip + the signal's
                meaning AND game consequence, re-explained at any of the four depths
            ...
            <View style={styles.nextRow}>
              <NextButton visible={chosen !== null} label="Next signal →" onPress={next} variant="filled" />
            </View>
          </>
        )}
      </ScrollView>
